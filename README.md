<h1 align="center">emischeduler</h1>
...
For more info about the package itself see `emischeduler/README.md`
...
(base) $ git clone https://github.com/radio-aktywne/emischeduler
(base) $ cd emischeduler
(base) $ conda env create -f environment.yml
(base) $ conda activate emischeduler
(emischeduler) $ cd emischeduler
(emischeduler) $ poetry install --extras dev
...
```sh
$ python3 -m pip install ./emischeduler
$ emischeduler
```
...
To install the package, you need to `cd` into `emischeduler` directory and run:
...
write the tests and put them in `emischeduler/tests`.
...
After that update the installation by running from `emischeduler` directory:
...
```sh
 docker build -t emischeduler .
```

To also run the container in one go, run:

```sh
docker build -t emischeduler . && docker run --rm -it emischeduler
```
