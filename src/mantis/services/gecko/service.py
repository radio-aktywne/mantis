...
class RecordsNamespace(GracyNamespace[Endpoint]):
    ...
    async def list(self, request: m.RecordsListRequest) -> m.RecordsListResponse:
        ...
        response = await self.get(
            f"{Endpoint.RECORDS}/{Serializable(request.event).model_dump()}",
            params=params,
        )
    ...
    async def download(
        ...
        response = await self._client.send(
            self._client.build_request(
                "GET",
                f"{Endpoint.RECORDS}/{Serializable(request.event).model_dump()}/{Serializable(request.start).model_dump()}",
            ),
            stream=True,
        )
